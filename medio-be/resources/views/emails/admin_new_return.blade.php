<!DOCTYPE html>
<html lang="id">
<head><meta charset="UTF-8"><title>Retur Baru — Optik Medio Admin</title>
<style>
...
<body><div class="wrap">
  ...
  <div class="body">
    <span class="badge">Retur Baru</span>
    <p style="font-size:15px;font-weight:700;margin:0 0 20px">Ada permintaan retur baru yang perlu diproses.</p>
    <div style="background:#f9f7f4;padding:16px;margin-bottom:16px">
      <div class="row"><span class="lbl">Pelanggan</span><span class="val">{{ $returnRequest->user->name }}</span></div>
      <div class="row"><span class="lbl">Email</span><span class="val">{{ $returnRequest->user->email }}</span></div>
      @if($returnRequest->order)<div class="row"><span class="lbl">Order</span><span class="val">#{{ $returnRequest->order->order_number }}</span></div>@endif
      <div class="row"><span class="lbl">Alasan</span><span class="val">{{ $returnRequest->reason }}</span></div>
    </div>
    <p style="font-size:12px;text-transform:uppercase;letter-spacing:.08em;color:#8a7a60;margin:0 0 8px">Deskripsi</p>
    <div class="msg">{{ $returnRequest->description }}</div>
    <a href="{{ config('app.url') }}/admin/return-requests/{{ $returnRequest->id }}/edit" class="cta">Proses Retur →</a>
  </div>
