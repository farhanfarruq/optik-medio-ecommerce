...
<template>
  ...
  <main class="max-w-[1000px] mx-auto w-full px-6 pb-20 flex-grow" style="padding-top: 120px;">

    <!-- Empty Cart -->
    <div v-if="cartStore.items.length === 0" class="text-center py-24 rounded-3xl border border-dashed" style="border-color: rgba(193,154,81,0.25); background: rgba(193,154,81,0.04);">
      ...
      <router-link
        to="/products"
        class="inline-flex items-center gap-2 px-8 py-3.5 rounded-2xl font-black text-sm text-white transition-all hover:shadow-xl active:scale-95"
    ...
    <div v-else class="flex flex-col md:flex-row gap-8">
      ...
      <div class="flex-grow flex flex-col gap-5">
        <div
          ...
          :key="item.cart_id"
          class="relative rounded-2xl border overflow-hidden transition-all hover:shadow-md"
          style="background: white; border-color: rgba(193,154,81,0.12);"
        >
          <div class="flex gap-5 p-5">
            <!-- Image -->
            <div class="w-28 h-28 rounded-xl overflow-hidden shrink-0 flex items-center justify-center p-2 border" style="background: linear-gradient(145deg, #f5f2ee, #ede7dc); border-color: rgba(193,154,81,0.1);">
            ...
            <div class="flex flex-col flex-grow min-w-0">
              ...
              <!-- Prescription -->
              <div v-if="item.prescription" class="mt-3 p-3 rounded-xl text-xs flex flex-col gap-1" style="background: rgba(193,154,81,0.06); border: 1px solid rgba(193,154,81,0.12);">
      ...
      <div class="w-full md:w-72 shrink-0">
        <div class="rounded-2xl border p-6 sticky top-24" style="background: white; border-color: rgba(193,154,81,0.15); box-shadow: 0 4px 20px rgba(0,0,0,0.05);">
          ...
          <button
            ...
            class="w-full py-4 rounded-2xl font-black text-sm text-white uppercase tracking-wider flex items-center justify-center gap-2 transition-all hover:shadow-xl active:scale-95"
            style="background: linear-gradient(135deg, #1a1209 0%, #3d2c0e 100%);"
          >
            Lanjut ke Checkout
