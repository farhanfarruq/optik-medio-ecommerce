  ...
  <div class="relative w-full bg-[#f5f2ee]" style="margin-bottom: -60px;">
    <div class="relative overflow-hidden" style="height: 280px;">
      ...
      <div class="relative z-10 h-full max-w-[1440px] mx-auto px-6 md:px-12 flex flex-col justify-between" :style="{ paddingTop: 'calc(var(--header-height, 96px) + 16px)', paddingBottom: '56px' }">
        ...
        <div>
          <p class="text-[10px] font-black uppercase tracking-[0.3em] mb-3" style="color: #c19a51;">Face Shape Quiz</p>
          <h1 class="text-4xl md:text-5xl font-black tracking-tight text-white" style="font-family: 'Outfit', sans-serif;">Cari Frame yang Cocok</h1>
        </div>
      </div>
    </div>
  </div>
  ...
  <main class="min-h-screen bg-[#f5f2ee] pb-24 pt-24">
    <section class="max-w-[1440px] mx-auto px-6 md:px-12">
      <div class="grid grid-cols-1 lg:grid-cols-[420px_1fr] gap-8">
        <aside class="bg-white border border-stone-100 p-6 h-fit sticky top-28">
          <p class="text-[10px] font-black uppercase tracking-[0.3em] mb-3" style="color: #c19a51;">Quiz Cepat</p>
          <h2 class="text-3xl font-black tracking-tight mb-4" style="color: #1a1209; font-family: 'Outfit', sans-serif;">Preferensi Frame</h2>
          <p class="text-sm leading-relaxed text-stone-500 mb-6">Jawab empat pilihan cepat untuk mendapatkan rekomendasi frame yang lebih relevan.</p>

          <div class="space-y-5">
            <label class="block">
              <span class="block text-[10px] font-black uppercase tracking-widest text-stone-500 mb-2">Bentuk Wajah</span>
              <select v-model="faceShape" class="w-full border border-stone-300 bg-white px-3 py-3 text-sm focus:outline-none focus:border-amber-700">
            ...
            <label class="block">
              <span class="block text-[10px] font-black uppercase tracking-widest text-stone-500 mb-2">Style</span>
              <select v-model="stylePreference" class="w-full border border-stone-300 bg-white px-3 py-3 text-sm focus:outline-none focus:border-amber-700">
                <option value="">Pilih</option>
            ...
            <label class="block">
              <span class="block text-[10px] font-black uppercase tracking-widest text-stone-500 mb-2">Ukuran Wajah</span>
              <select v-model="faceSize" class="w-full border border-stone-300 bg-white px-3 py-3 text-sm focus:outline-none focus:border-amber-700">
                <option value="">Pilih</option>
            ...
            <label class="block">
              <span class="block text-[10px] font-black uppercase tracking-widest text-stone-500 mb-2">Budget</span>
              <select v-model="budget" class="w-full border border-stone-300 bg-white px-3 py-3 text-sm focus:outline-none focus:border-amber-700">
                <option value="">Pilih</option>
        ...
        <section class="min-w-0">
          <div v-if="!hasSubmitted" class="bg-white border border-stone-100 p-10 min-h-[420px] flex flex-col justify-center">
            <span class="material-symbols-outlined text-6xl mb-5" style="color: #c19a51;">face</span>
            <h2 class="text-3xl font-black mb-3" style="color: #1a1209; font-family: 'Outfit', sans-serif;">Mulai dari bentuk wajah</h2>
            <p class="text-stone-500 max-w-xl">Quiz ini memakai atribut produk optik yang sudah ada: bentuk frame, material, fit wajah, stok, dan rentang harga.</p>
          </div>

          <div v-else>
            <div class="bg-white border border-stone-100 p-6 mb-6">
              <p class="text-[10px] font-black uppercase tracking-[0.3em] mb-2" style="color: #c19a51;">Hasil</p>
              <h2 class="text-2xl font-black mb-2" style="color: #1a1209; font-family: 'Outfit', sans-serif;">
                Rekomendasi {{ selectedFace?.label }}
              </h2>
              <p class="text-sm text-stone-500">{{ selectedFace?.note }}</p>
            ...
            <div v-if="isLoading" class="grid grid-cols-2 md:grid-cols-3 gap-5">
              <div v-for="item in 6" :key="item" class="animate-pulse bg-white border border-stone-100">
            ...
            <div v-else-if="results.length === 0" class="bg-white border border-stone-100 p-10 text-center">
              <h3 class="text-xl font-black mb-2" style="color: #1a1209;">Belum ada produk cocok</h3>
              <p class="text-sm text-stone-500">Coba ubah budget atau ukuran wajah.</p>
            </div>
            ...
            <div v-else class="grid grid-cols-2 md:grid-cols-3 gap-5">
              ...
              >
                ...
                <div class="p-4">
                  ...
                  <h3 class="text-sm font-black line-clamp-2 min-h-10" style="color: #1a1209;">{{ product.name }}</h3>
