  ...
  <div class="relative w-full bg-[#f5f2ee]" style="margin-bottom: -60px;">
    <div class="relative overflow-hidden" style="height: 280px;">
      ...
      <div class="relative z-10 h-full max-w-[1440px] mx-auto px-6 md:px-12 flex flex-col justify-between" :style="{ paddingTop: 'calc(var(--header-height, 96px) + 16px)', paddingBottom: '56px' }">
        ...
        <div>
          <p class="text-[10px] font-black uppercase tracking-[0.3em] mb-3" style="color: #c19a51;">Compare</p>
          <h1 class="text-4xl md:text-5xl font-black tracking-tight text-white" style="font-family: 'Outfit', sans-serif;">Bandingkan Produk</h1>
        </div>
      </div>
    </div>
  </div>
  ...
  <main class="min-h-screen bg-[#f5f2ee] pb-24 pt-24">
    <section class="max-w-[1440px] mx-auto px-6 md:px-12">
      <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-5 mb-8">
        <div>
          <p class="text-sm font-bold text-stone-500">Pilih 2 sampai 4 produk untuk melihat perbedaan fitur, harga, dan rekomendasi.</p>
        </div>
        <div class="flex gap-3">
          <button @click="compareStore.clear()" class="px-4 py-2 border border-stone-300 text-xs font-black uppercase tracking-widest text-stone-700">
            Bersihkan
          </button>
          <button @click="router.push('/products')" class="px-4 py-2 text-xs font-black uppercase tracking-widest text-white" style="background: #1a1209;">
            Tambah Produk
          </button>
        </div>
      </div>
      ...
      <div v-if="compareStore.items.length < 2" class="border border-dashed p-10 text-center bg-white">
        <span class="material-symbols-outlined text-5xl mb-4 block" style="color: #c19a51;">compare_arrows</span>
        <h2 class="text-xl font-black mb-2" style="color: #1a1209;">Pilih minimal 2 produk</h2>
        <p class="text-sm text-stone-500 mb-6">Maksimal 4 produk bisa dibandingkan sekaligus.</p>
      ...
      <div v-else class="overflow-x-auto bg-white border border-stone-100 shadow-sm">
        <table class="min-w-full text-sm">
          <thead>
            <tr>
              <th class="w-48 p-4 text-left text-xs font-black uppercase tracking-widest text-stone-500 bg-stone-50">Atribut</th>
              <th v-for="product in products" :key="product.id" class="min-w-56 p-4 text-left align-top border-l border-stone-100">
                <button @click="router.push(`/products/${product.slug}`)" class="text-left group">
                  <img :src="resolveImageUrl(product)" :alt="product.name" class="w-24 h-24 object-contain bg-stone-50 border border-stone-100 mb-3" />
                  <p class="text-[10px] font-black uppercase tracking-widest text-stone-500">{{ product.brand || 'Optik Medio' }}</p>
                  <h2 class="font-black text-stone-900 group-hover:text-amber-800">{{ product.name }}</h2>
                </button>
                <button @click="compareStore.remove(product.id)" class="mt-3 text-xs font-bold text-red-600 underline">
                  Hapus
                </button>
              </th>
            </tr>
          ...
          <tbody>
            <tr v-if="isLoading">
              <td :colspan="products.length + 1" class="p-8 text-center text-stone-500">Memuat perbandingan...</td>
            </tr>
            <tr v-for="attribute in attributes" :key="attribute" class="border-t border-stone-100">
              <td class="p-4 font-black text-stone-700 bg-stone-50">{{ attributeLabels[attribute] }}</td>
              <td v-for="product in products" :key="product.id + attribute" class="p-4 border-l border-stone-100 text-stone-700 capitalize">
