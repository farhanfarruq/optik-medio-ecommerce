...
<template>
  <div>
    ...
    <main :class="props.embedded ? 'w-full' : 'container-commerce py-12'">
      ...
      <template v-else>
        <!-- Tabs -->
        <div class="flex gap-1 mb-8 border-b" style="border-color: var(--mist);">
          <button
        ...
        <div v-else-if="activeTab === 'warranties'">
          ...
          <div v-else class="space-y-4">
            <div v-for="w in warranties" :key="w.id" class="border p-6" style="background: white; border-color: rgba(184,138,68,0.15);">
              ...
              <button
                ...
                class="mt-5 inline-flex items-center justify-center gap-2 w-full px-4 py-3 text-xs font-black uppercase tracking-wider text-white transition-all hover:shadow-card"
                style="background: var(--ink);"
              >
                <span class="material-symbols-outlined text-sm">support_agent</span>
                Klaim Garansi
          ...
          <div v-else class="space-y-4">
            <div v-for="c in claims" :key="c.id" class="border p-5" style="background: white; border-color: rgba(184,138,68,0.15);">
              <div class="flex items-start justify-between gap-3 mb-2">
                <p class="font-bold text-sm" style="color: var(--ink);">{{ c.claim_number }}</p>
                ...
                <div v-if="c.images?.length">
                  ...
                  <div class="flex flex-wrap gap-2">
                    <a
                      ...
                      style="border-color: var(--mist); color: var(--ink);"
                    >
            ...
            <div>
              ...
              <div class="grid gap-2">
                <label v-for="ct in claimTypes" :key="ct.value" class="flex items-center gap-3 p-3 border cursor-pointer" :style="claimForm.claim_type === ct.value ? 'border-color: var(--gold); background: rgba(184,138,68,0.05);' : 'border-color: var(--mist);'">
                  ...
                  <span class="material-symbols-outlined text-lg" style="color: var(--gold);">{{ ct.icon }}</span>
                  <span class="text-sm font-medium" style="color: var(--ink);">{{ ct.label }}</span>
                </label>
              </div>
            </div>
